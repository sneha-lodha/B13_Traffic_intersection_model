...
class Car(Agent):
    ...
    def moveForward(self, x, y):
        # move in x direction, keep y direction
    ...
    def move(self):
        if self.direction == 'east':
            if self.can_move(
                    self.model.grid.width - 1,
                    0):		# if the car can/is allowed to move
                if self.turnAhead():														# if car has to turn
                    if self.distance < self.model.grid.width / \
                            2:  # if the car has to turn right of left
                        # the car has to turn right
                        self.turn('south', 0, -1)
                    else:
                        # the  car has to turn left
                        self.turn('north', 0, 1)
                else:
                    # the car moves 1 cell forward
